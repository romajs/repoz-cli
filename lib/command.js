...
function exec(command, project, args, successCallback, errorCallback) {
	...
	if(!fs.existsSync(f)) {
		...
		vault = new credentials.Vault(f, cipher_key);
		vault.save();
	} else {
	...
	vault.get(project, ACCESS_TYPES[command]).then(function(credential) {

		project = new repoz.Project(credential.project, credential.username, credential.password);

		project[command].apply(this, args).then(function() {
